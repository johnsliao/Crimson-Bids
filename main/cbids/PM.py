...
class IPhoneVault:
    ...
    def getResultsHeader(self):
        #8 chars per entry with 2 spaces recommended
        header = 'VIABL   CONDI   FUNCT   IMEI    ESN     iCLD    ACCES'
        print header
        return header

    # ACCES has values CBPCHCM for all accessories, * for missing one. (C)harger (Box) (P)rotector (C)ase (H)eadphone (C)able (M)anual
    def getAccessoriesEncoded(self):
        encod = ''
        for k in self.accessories:
            if self.accessories[k]=='YES':
                encod +=k[0]
            else:
                encod+='*'
        return encod
    ...
    def getResults(self):
        ...
        #Condition is { 'UNDEFINED', '???', 'BAD', 'GOOD' }
        Cond='??? '  #will treat like UNDEFINED
        if 'BAD' == self.physicalCondition :
            Cond = 'BAD '
        if 'GOOD' == self.physicalCondition :
            Cond = 'GOOD'

        #accessories are listed in one string with first letter or '*'
        Encod = self.getAccessoriesEncoded()

        #IMEI, ESN, and iCloud are locked/clearn
        if self.IMEI=='LOCKED':
            istatus = 'LOCKED'
        else:
            istatus = 'CLEAN '
        ...
        if self.iCloud =='LOCKED':
            cstatus = 'LOCKED'
        else:
            cstatus = 'CLEAN '

        #borrow logic from getStatus()

        Functionality = 'GOOD'
        if 'BAD' in self.functionalityCondition.values():
            Functionality = 'BAD '

        results = Viable+ "     " + Cond + "    " + Functionality+"    "+ istatus+"  "+estatus+"  " + cstatus + "  " + Encod
        print results
        return results
...
class IPhoneDictionary:
    ...
    def vaultHeader(self):
        return self.IPhoneVault.getResultsHeader()
    def vaultResults(self):
        return self.IPhoneVault.getResults()
    ...
    def interpretText(self, inputText): # run text through conditionalClasses
        ...
        self.IPhoneVault.getResultsHeader()
        self.IPhoneVault.getResults()
    ...
    def push(self, sentence):
        ...
        for wordPosition, word in enumerate(sentence):
            ...
            for accessory in self.accesories:
                ...
                if accessory.lower() in word and not self.hasNegation(sentence, wordPosition):
